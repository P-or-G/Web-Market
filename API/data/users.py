...
class User(SqlAlchemyBase):
    ...
    current_id = sqlalchemy.Column(sqlalchemy.String, nullable=True)
    history_id = sqlalchemy.Column(sqlalchemy.String, nullable=True)

    goods = orm.relationship("Goods", back_populates='user')

    def __repr__(self):
        return f'{self.id} *** {self.login} *** {self.email} *** {self.hashed_password} *** {self.status} *** ' \
               f'{self.telegram_id} *** {self.current_id} *** {self.history_id}'
